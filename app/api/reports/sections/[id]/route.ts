...
// PUT /api/reports/sections/[id] — update a section's name or position
export async function PUT(
  ...
  try {
    ...
    console.error("PUT /api/reports/sections/[id] error:", err)
...
export async function DELETE(
  ...
  try {
    ...
    const admin = createAdminClient()
    const { error } = await admin.from("report_sections").delete().eq("id", sectionId)
    if (error) throw error
    return NextResponse.json({ success: true })
  } catch (err) {
    console.error("DELETE /api/reports/sections/[id] error:", err)
    return NextResponse.json({ error: "Internal server error" }, { status: 500 })
  }
}
